-- MySQL Script generated by MySQL Workbench
-- 03/06/15 01:03:24
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='TRADITIONAL,ALLOW_INVALID_DATES';

-- -----------------------------------------------------
-- Schema mydb
-- -----------------------------------------------------

-- -----------------------------------------------------
-- Schema mydb
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `mydb` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci ;
-- -----------------------------------------------------
-- Schema westoque
-- -----------------------------------------------------
USE `mydb` ;

-- -----------------------------------------------------
-- Table `mydb`.`usuario`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`usuario` (
  `idusuario` TINYINT(11) NOT NULL AUTO_INCREMENT COMMENT 'Tabela para usuario',
  `usuario` VARCHAR(50) NOT NULL,
  `nomeusuario` VARCHAR(100) NOT NULL,
  `emailusuario` VARCHAR(100) NOT NULL,
  `senhausuario` VARCHAR(100) NOT NULL,
  PRIMARY KEY (`idusuario`),
  UNIQUE INDEX `emailusuario_UNIQUE` (`emailusuario` ASC))
ENGINE = MyISAM
AUTO_INCREMENT = 1
DEFAULT CHARACTER SET = latin1;


-- -----------------------------------------------------
-- Table `mydb`.`fornecedor`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`fornecedor` (
  `idfornecedor` TINYINT(11) NOT NULL AUTO_INCREMENT COMMENT 'Tabela do fornecedor\n',
  `cnpjfornecedor` VARCHAR(20) NOT NULL,
  `razaosocialfornecedor` VARCHAR(100) NOT NULL,
  `emailfornecedor` VARCHAR(100) NOT NULL,
  `telefonefornecedor` VARCHAR(100) NOT NULL,
  UNIQUE INDEX `cnpjfornecedor_UNIQUE` (`cnpjfornecedor` ASC),
  PRIMARY KEY (`idfornecedor`))
ENGINE = MyISAM
AUTO_INCREMENT = 1
DEFAULT CHARACTER SET = latin1;


-- -----------------------------------------------------
-- Table `mydb`.`categoria`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`categoria` (
  `idcategoria` INT(11) NOT NULL AUTO_INCREMENT COMMENT 'Tabela para categoria',
  `nomecategoria` VARCHAR(30) NOT NULL,
  PRIMARY KEY (`idcategoria`))
ENGINE = InnoDB
AUTO_INCREMENT = 1
DEFAULT CHARACTER SET = latin1;


-- -----------------------------------------------------
-- Table `mydb`.`produto`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`produto` (
  `idproduto` TINYINT(11) NOT NULL AUTO_INCREMENT COMMENT 'tabela para produto',
  `nomeproduto` VARCHAR(50) NOT NULL,
  `dataproduto` DATE NULL DEFAULT NULL,
  `idcategoria` INT(11) NOT NULL,
  PRIMARY KEY (`idproduto`),
  INDEX `fk_produto_categoria_idx` (`idcategoria` ASC))
ENGINE = MyISAM
AUTO_INCREMENT = 1
DEFAULT CHARACTER SET = latin1;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;
